...
int IOSRendererBackend::getWidth() {
    auto width = static_cast<int>(roundf(eagLayer.bounds.size.width * eagLayer.contentsScale));
    return width;
}

int IOSRendererBackend::getHeight() {
    auto height = static_cast<int>(roundf(eagLayer.bounds.size.height * eagLayer.contentsScale));
    return height;
}
